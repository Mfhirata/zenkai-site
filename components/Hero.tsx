...
interface HeroProps {
  ...
  // Novas props para dados editáveis (vão vir do JSON em breve)
  title?: string;
  description?: string;
  heroImage?: string;
}
...
const Hero: React.FC<HeroProps> = ({
  ...
  onContactClick,
  onQuickEstimate,
  title = "REPROGRAMAÇÃO DE CENTRALINA <br /> & CHIPTUNING MASTER PORTUGAL",  // fallback hardcoded
  description = "Especialista em Remap Stage 1 & Stage 2, IMMO OFF e Soluções ECU. Otimização WinOLS Master para clientes presenciais em Portugal e suporte técnico para preparadores em toda a Europa.",
  heroImage = "https://images.unsplash.com/photo-1614162692292-7ac56d7f7f1e?auto=format&fit=crop&q=80&w=2070"  // fallback
}) => {
  ...
  return (
    // Wrapper com object-id para scope de todo o hero (edita o documento inteiro)
    <section
      className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20"
      data-sb-object-id="home-page"  // ID único do objeto (liga ao JSON home.json)
    >
      <div
        className="absolute inset-0 z-0 overflow-hidden"
        style={{ transform: `translateY(${scrollY * 0.4}px)` }}
      >
        {/* Imagem hero editável: permite trocar a imagem no editor */}
        <img
          src={heroImage}
          ...
          data-sb-field-path="heroImage#@src"  // Campo editável para o src da imagem
        />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 text-center py-10 md:py-20">
        ...
        <h1
          className="text-3xl md:text-6xl font-display font-bold mb-8 tracking-tight leading-[1.1] text-white uppercase italic"
          data-sb-field-path="title"  // Campo editável
          dangerouslySetInnerHTML={{ __html: title }}  // Suporta <br /> no título
        />

        {/* Descrição editável */}
        <p
          className="text-lg md:text-xl text-gray-300 mb-12 max-w-3xl mx-auto font-light text-balance px-2 leading-relaxed"
